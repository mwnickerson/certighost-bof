...
static void test_truncated_pack(void) {
    ...
    expect_status("truncated payload rejected", cg_parse_packed_args(packed, packed_len - 1u, &input), CG_ERR_PACK_TRUNCATED);
    expect_status("truncated header rejected", cg_parse_packed_args(packed, 3u, &input), CG_ERR_PACK_HEADER);
}
...
static void test_trailing_pack(void) {
    ...
    expect_status("trailing bytes rejected", cg_parse_packed_args(packed, packed_len + 1u, &input), CG_ERR_PACK_TRAILING);
}
